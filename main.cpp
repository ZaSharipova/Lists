...
#define DoChange(func, list, pos, number, type_of_change)                          \
    ...
    snprintf(img_name, sizeof(img_name), "graph_%d.svg", graph_counter++); \
...
int main(void) {
    ...
    DoChange(InsertElement(&list, 0, 32), list, 0, 32.0, kInsert);

    DoChange(InsertElement(&list, 1, 20), list, 1, 20.0, kInsert);

    DoChange(InsertElement(&list, 2, 25), list, 1, 25, kInsert);

    DoChange(InsertElement(&list, 3, 26), list, 3, 26, kInsert);
